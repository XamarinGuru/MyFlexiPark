...
namespace UICalendar
{
    ...
    public class RotatingCalendarView : RotatingViewController
    {
        ...
        public RotatingCalendarView (CGRect rect)
            : this (rect, 0)
        {
        }
        ...
        public RotatingCalendarView (CGRect rect, float tabBarHeight)
        {
            ...
            SingleDayView.ForceAutoRotate = delegate {
                ForceAutoRotate ();
            };
            ...
            this.OnEventClicked += theEvent => {
                ...
                //Duy commented out , cause this is not used anymore
    ...
    public delegate void EditEventClicked (CalendarDayEventView theEvent);
    public delegate void DeleteEventClicked (CalendarDayEventView theEvent);
    ...
    public class CalendarDayEventView : UIView
    {
        ...
        public OwnerUnavailability _Event { get; set; }

        public Reservation _Booking { get; set; }

        public CalendarDayEventView (OwnerUnavailability _event)
        {
            if (_event != null) {
                _Event = _event;
                this.Title = _event.Title;
                this.IsBooking = false;
                this.eventIdentifier = _event.UnavailabilityId;
                this.startDate = _event.StartTimestamp.UnixTimeStampToDateTime ();
                this.endDate = _event.EndTimestamp.UnixTimeStampToDateTime ();
        ...
        public CalendarDayEventView (Reservation _booking)
        {
            if (_booking != null) {
                _Booking = _booking;
                this.Title = _booking.PlateNumber;
                this.IsBooking = true;
                this.eventIdentifier = _booking.BookingId;
                this.startDate = _booking.StartTimestamp.UnixTimeStampToDateTime ();
                this.endDate = _booking.EndTimestamp.UnixTimeStampToDateTime ();
                TimeSpan dateDif = endDate - startDate;
                if (dateDif.Minutes < 30 && dateDif.Minutes > 1) {
                    endDate = endDate.AddMinutes (30 - dateDif.Minutes);
                }
    ...
    public class CalendarDayTimelineView : UIView
    {
        ...
        public static List<CalendarDayEventView> monthEvents = new List<CalendarDayEventView> ();
        ...
        public void ForceReloadView (bool shouleRefreshUI, bool eventsNeedRefresh)
        {
            ShouldRefreshUI = shouleRefreshUI;
            EventsNeedRefresh = eventsNeedRefresh;
            reloadDay ();
        }
        ...
        private void LoadButtons ()
        {
            ...
            _leftButton.TouchUpInside += delegate {
                SetDate (currentDate.AddDays (-1));
            };
            ...
            _rightButton.TouchUpInside += delegate {
                SetDate (currentDate.AddDays (1));
            };
        ...
        private UIView getScrollView ()
        {
            ...
            parentScrollView.BackgroundColor = UIColor.Clear;
            setBottomToolBar ();
        ...
        {
            CGRect frame = this.Frame;
            //CGPoint pointInTabbar = bottomBar.ConvertPointFromView (point, this);
            //bool shit = bottomBar.PointInside (pointInTabbar, uievent);
            //if (shit)
            //    return bottomBar;
        ...
        private void setBottomToolBar ()
        {
            ...
            bottomBar = new UIToolbar (new CGRect (scrollFrame.X, scrollFrame.Height + 4 + 40, scrollFrame.Width, bottomBarH + 40)) { TintColor = UIColor.Black };
            calViewSwitcher = new UISegmentedControl (new CGRect (scrollFrame.Width / 2 - 90, (bottomBarH + 20 - 30) / 2, 180, 30));
            ...
            calViewSwitcher.ValueChanged += delegate {
                Settings.lastCal = calViewSwitcher.SelectedSegment;
                ViewSwitched ();
            };
            ...
            UIButton btnToday = new UIButton (UIButtonType.Custom);
            btnToday.SetTitle ("Today", UIControlState.Normal);
            btnToday.SetTitleColor (UIColor.Black, UIControlState.Normal);
            btnToday.Frame = new CGRect (5, (bottomBarH + 20 - 40) / 2, 80, 40);
        ...
        public void buildDayView ()
        {
            ...
            //var dailyEvents = monthEvents.Where(x => (x.startDate.Date == currentDate || x.endDate.Date == currentDate) && !x.AllDay && (x.endDate.Date != currentDate)).ToList();

            var dailyEvents = monthEvents.Where (x => ((x.startDate.Date == currentDate || x.endDate.Date == currentDate) && (x.endDate.Date != currentDate)) || (x.startDate.Date == currentDate && x.endDate.Date == currentDate)).OrderBy (x => x.startDate).ToList ();
        ...
        public static CalendarMonthView calMonthView;
        ...
        public void buildMonthView ()
        {
            ...
            calMonthView.IsDayMarkedDelegate += date => {
                return monthEvents.Where (x => x.startDate.Date == date.Date || x.endDate.Date == date.Date).Count () > 0;
            };
        ...
        public RootElement getMonthDayEvents ()
        {
            ...
            var tmp = monthEvents.Where (x => ((x.startDate.Date == currentDate || x.endDate.Date == currentDate) && (x.endDate.Date != currentDate)) || (x.startDate.Date == currentDate && x.endDate.Date == currentDate)).OrderBy (x => x.startDate).ToList ();
            foreach (var theEvent in tmp) {
                var theelement = new MonthEventElement (theEvent);
                theelement.OnEventClicked += theClickedEvent => {
                    if (OnEventClicked != null)
                        OnEventClicked (theClickedEvent);
                };
                theelement.OnEditEventClicked += theClickedEvent => {
                    if (OnEditEventClicked != null)
                        OnEditEventClicked (theClickedEvent);
                };
                theelement.OnDeleteEventClicked += theClickedEvent => {
                    if (OnDeleteEventClicked != null)
                        OnDeleteEventClicked (theClickedEvent);
                };
