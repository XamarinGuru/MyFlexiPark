...
{
    public class BookingToWaitConverter : MvxValueConverter<int,string>
    {
        protected override string Convert(int value, Type targetType, object parameter, System.Globalization.CultureInfo culture)
        {
            return string.Format("{0}' {1}", value, Mvx.Resolve<ICacheService>().TextSource.GetText("ToWaitText"));
        }
    }
}
